...
from dotenv import load_dotenv
from langfuse import get_client, observe  # ✅ New imports
from livekit import rtc
from livekit.agents import (
    Agent,
    AgentSession,
...
class VoiceAgent(Agent):
    """A voice-only agent with STT, LLM, and TTS capabilities."""
    ...
    async def on_exit(self) -> None:
        """Called when the agent is about to leave."""
        await self.session.generate_reply(instructions="say goodbye briefly")
        await self.close()
        logger.info("VoiceAgent exited session.")
    ...
    # ---------------------------------------------------------
    # Nodes (Decorated for Langfuse tracing)
    # ---------------------------------------------------------
    ...
    @observe(name="tts_node")
    async def tts_node(
        self, text: AsyncIterable[str], model_settings: ModelSettings
    ) -> AsyncIterable[rtc.AudioFrame]:
        """Convert text to speech with Sarvam TTS."""
        try:
            async for frame in Agent.default.tts_node(self, text, model_settings):
                yield frame
        except Exception as e:
...
@observe(name="voice_agent_session")
async def entrypoint(ctx: JobContext) -> None:
    """Main entry point for the VoiceAgent (traced as one session)."""
    await ctx.connect()
    logger.info(f"Connected to room: {ctx.room.name}")
